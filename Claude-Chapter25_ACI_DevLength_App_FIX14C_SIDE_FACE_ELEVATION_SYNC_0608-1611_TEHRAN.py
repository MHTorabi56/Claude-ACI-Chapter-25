...
def inject_custom_css() -> None:
    ...
    This function changes visual styling only and does not modify calculation logic.
    """
    st.markdown(
        """
<style>
:root {
    ...
    --aci-bg1: #0a1726;
    --aci-bg2: #0e2033;
    --aci-card: rgba(18, 36, 55, 0.92);
    --aci-card2: rgba(8, 22, 36, 0.94);
    --aci-border: rgba(120, 170, 220, 0.22);
    --aci-text: #eaf3ff;
    --aci-muted: #9fb2c8;
    --aci-blue: #1c86ff;
    --aci-cyan: #28d5ff;
...
html, body, [data-testid="stAppViewContainer"] {
    background:
        radial-gradient(circle at 12% 0%, rgba(28,134,255,0.18), transparent 32%),
        radial-gradient(circle at 100% 12%, rgba(40,213,255,0.10), transparent 30%),
        linear-gradient(180deg, var(--aci-bg0), var(--aci-bg1)) !important;
...
[data-testid="stSidebar"] {
    background:
        linear-gradient(180deg, rgba(6,17,29,0.98), rgba(8,23,38,0.98)) !important;
...
.aci-hero {
    ...
    margin: 8px 0 18px 0;
    background:
        linear-gradient(135deg, rgba(16, 43, 70, .95), rgba(6, 17, 29, .96));
...
.aci-logo {
    ...
    border-radius: 16px;
    margin-bottom: 12px;
    background: linear-gradient(135deg, #0f70ff, #28d5ff);
    font-weight: 900;
    font-size: 20px;
    box-shadow: 0 0 28px rgba(28,134,255,.36);
}
...
.aci-chip.ok { color: #baffc9; border-color: rgba(52,227,111,.36); }
.aci-chip.warn { color: #ffd58a; border-color: rgba(255,176,32,.36); }
.aci-chip.info { color: #cde7ff; border-color: rgba(28,134,255,.45); }
...
div[data-testid="stTabs"] button[aria-selected="true"] {
    ...
    border-bottom-color: var(--aci-cyan) !important;
}

.stButton>button {
    border-radius: 12px;
    background: linear-gradient(135deg, #0f70ff, #1250cf);
    color: white;
    border: 1px solid rgba(80,160,255,.65);
    font-weight: 800;
}
...
hr { border-color: rgba(120,170,220,.16) !important; }

/* -----------------------------------------------------------------------
   FIX8A — Dark UI text contrast fix
   Scope: visual/readability only. No calculation logic is modified.
   ----------------------------------------------------------------------- */

/* Streamlit widget labels: number_input, text_input, selectbox, sliders, etc. */
...
/* Captions/help text: keep readable on dark cards. */
...
/* Markdown body text and dataframe captions inside dark background. */
...
/* Input boxes: avoid light field on dark UI and keep typed values visible. */
div[data-testid="stNumberInput"] input,
div[data-testid="stTextInput"] input,
div[data-testid="stDateInput"] input,
div[data-testid="stTimeInput"] input,
textarea {
    background-color: rgba(4, 14, 25, 0.92) !important;
    color: #ffffff !important;
    border: 1px solid rgba(120, 170, 220, 0.45) !important;
...
/* Selectbox / multiselect visible values. */
div[data-baseweb="select"] > div,
div[data-baseweb="select"] div,
div[data-baseweb="popover"] div {
    background-color: rgba(4, 14, 25, 0.96) !important;
    color: #ffffff !important;
    border-color: rgba(120, 170, 220, 0.45) !important;
}

/* Disabled/secondary text should not disappear. */
[aria-disabled="true"],
[disabled],
...
/* -----------------------------------------------------------------------
   FIX9 — Professional typography, button states, and contrast polish
   Scope: UI readability only. No calculation logic is modified.
   ----------------------------------------------------------------------- */
...
.stDownloadButton > button:hover {
    ...
    box-shadow: 0 10px 28px rgba(28,134,255,.26) !important;
    border-color: rgba(40,213,255,.72) !important;
}
...
button[role="tab"][aria-selected="true"] {
    background: rgba(28,134,255,.16) !important;
    box-shadow: inset 0 -2px 0 #28d5ff !important;
}

[data-testid="stDataFrame"] {
    border: 1px solid rgba(120,170,220,.22) !important;
    border-radius: 14px !important;
    overflow: hidden !important;
    background: rgba(4,14,25,.60) !important;
}
...
/* Expander headers and tab labels. */
...
/* FIX10D — remove icon-text collisions and keep section cards readable.
   UI-only: no ACI calculation logic is modified. */
[data-testid="stExpander"] summary,
[data-testid="stExpander"] summary * {
...
.aci-section-title {
    ...
    padding: 10px 14px !important;
    border-radius: 12px !important;
    background: linear-gradient(135deg, rgba(28,134,255,.18), rgba(40,213,255,.07)) !important;
    border: 1px solid rgba(40,213,255,.26) !important;
    color: #ffffff !important;
...
/* ACI 25.x expander header: force readable dark state in normal/open/hover/focus. */
...
[data-testid="stExpander"] summary:active {
    background: linear-gradient(135deg, #0f2940 0%, #071827 100%) !important;
    color: #f5fbff !important;
    -webkit-text-fill-color: #f5fbff !important;
    border: 1px solid rgba(84, 207, 255, .55) !important;
    border-radius: 13px !important;
    min-height: 46px !important;
    padding: 10px 14px !important;
    box-shadow: inset 0 0 0 1px rgba(255,255,255,.035), 0 8px 22px rgba(0,0,0,.22) !important;
}
[data-testid="stExpander"] summary *,
[data-testid="stExpander"] details > summary *,
[data-testid="stExpander"] details[open] > summary *,
...
[data-testid="stExpander"] details > summary svg * {
    ...
    fill: #54cfff !important;
    stroke: #54cfff !important;
    opacity: 1 !important;
}

/* Expander content panel should not create a white bar under the opened header. */
[data-testid="stExpander"] [data-testid="stExpanderDetails"],
[data-testid="stExpander"] details > div,
[data-testid="stExpander"] details[open] > div {
    background: rgba(7, 24, 39, .58) !important;
    color: #edf7ff !important;
    border: 1px solid rgba(84,207,255,.20) !important;
    border-top: 0 !important;
    border-radius: 0 0 13px 13px !important;
}

/* Generic light artifacts inside explanatory/reference tables/cards. */
...
[data-testid="stTable"] td {
    background-color: rgba(7, 24, 39, .86) !important;
    color: #edf7ff !important;
    -webkit-text-fill-color: #edf7ff !important;
    border-color: rgba(84,207,255,.24) !important;
}
[data-testid="stMarkdownContainer"] th,
[data-testid="stTable"] th {
    background-color: rgba(16, 41, 65, .96) !important;
...
/* Remove isolated white patches from code/math/table helper spans. */
[data-testid="stMarkdownContainer"] span,
[data-testid="stMarkdownContainer"] p,
...
[data-testid="stMarkdownContainer"] code {
    background: rgba(8, 22, 36, .92) !important;
    color: #eaf6ff !important;
    border: 1px solid rgba(84,207,255,.20) !important;
}

/* DataFrame/grid fallback. */
[data-testid="stDataFrame"],
[data-testid="stDataFrame"] * {
    color: #edf7ff !important;
    -webkit-text-fill-color: #edf7ff !important;
...
/* Keep number/select/text inputs readable without turning the stepper area into a white block. */
div[data-baseweb="input"] > div,
div[data-baseweb="select"] > div,
input, textarea {
    background-color: rgba(7, 24, 39, .88) !important;
    color: #f5fbff !important;
    -webkit-text-fill-color: #f5fbff !important;
    border-color: rgba(174,221,255,.70) !important;
}
button[aria-label="Increment"],
button[aria-label="Decrement"] {
    background: rgba(15, 41, 64, .94) !important;
    color: #eaf6ff !important;
    border-color: rgba(174,221,255,.34) !important;
}


/* FIX13 — terminology note style */
.geo-help code {
    background: rgba(8, 22, 36, .95) !important;
...
/* FIX14 — professional geometry layout overrides */
.geo-card {
...
.geo-result-card:hover {
    border-color: rgba(40,180,255,.45) !important;
...
<div style='text-align:center; margin-top:40px;'>
    <h1 style='margin-bottom:5px;'>ACI 318 – Chapter 25</h1>
    <h3 style='font-weight:normal;'>Development, Anchorage, and Splicing of Reinforcement</h3>
    <p style='margin-top:30px;'>Revision: 00</p>
    <p style='margin-top:25px;'><strong>Prepared by: M. H. Torabi</strong><br>RAJAN PETRO FARAYAND</p>
    <p style='margin-top:40px; width:70%; margin-left:auto; margin-right:auto;'>
        This interface provides calculation tools for development length, anchorage,
        and lap splices of reinforcement in accordance with
        <strong>ACI 318-2019</strong> and <strong>ACI 318-2025</strong>, Chapter 25.
    </p>
</div>
"""
...
<text x="34" y="42" fill="#f4f8ff" font-size="18" font-weight="700">Longitudinal Beam Section</text><text x="742" y="42" fill="#34e36f" font-size="14" font-weight="700">LIVE UPDATE</text>
